...
(() => {
  ...
  function frameConfig(settings, frame, time, qualityScale = 1) {
    ...
    return {
      ...
      warpDensity: 25e-4 + settings.warpDensity / 100 * 0.014,
  ...
  function layeredWarp(x, y, time, config) {
    ...
    const drift = time * 18e-5 * config.warpSpeed;
    ...
    render(settings, time, qualityScale = 1) {
      ...
      if (settings.compressionInterferencePatterns) {
        ...
        for (let y = Math.abs(config.temporalSeed) % spacing - spacing; y < height; y += spacing) {
          ...
          const offset = Math.sin(y * 0.073 + time * 17e-4 + config.temporalPhase) * config.jitterPx + warp;
      ...
      if (settings.edgeReconstructionPoisoning) {
        ...
        for (let x = -block; x < width + block; x += block) {
          const phase = Math.sin(x * 0.031 + time * 11e-4) * config.edgePx + layeredWarp(x, 0, time, config) * 0.55;
        ...
        for (let y = -block; y < height + block; y += block) {
          const phase = Math.cos(y * 0.027 + time * 13e-4) * config.edgePx + layeredWarp(0, y, time, config) * 0.55;
...
void main(){ gl_Position = vec4(verts[gl_VertexID], 0.0, 1.0); }`;
  ...
  var WebGLPerturbationRenderer = class {
    ...
    render(settings, time, qualityScale = 1) {
      ...
      gl.uniform1f(this.uniforms.uJitter, settings.reducedMotion ? settings.jitter * 14e-4 : settings.jitter * 75e-4);
